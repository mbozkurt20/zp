    ...
    <div class="py-5 dark:text-white/50 bg-gray-900">
      <div class="relative flex min-h-screen flex-col w-full  mx-auto max-w-7xl">
        <div class="relative w-full">
          <main class="px-4 sm:px-8 py-12 pb-24">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
              <div
                  v-for="category in categories"
                  :key="category.id"
                  @click="$inertia.visit(`/category/${category.id}`)"
                  class="cursor-pointer bg-white dark:bg-gray-800 rounded-xl p-6 shadow-md hover:shadow-lg hover:bg-pink-50 dark:hover:bg-pink-900 transition duration-300 flex flex-col justify-center items-center text-pink-700 dark:text-pink-400"
              >
                <h3 class="font-semibold text-2xl mb-2">{{ category.name }}</h3>
                <p class="text-sm text-pink-400 dark:text-pink-300 opacity-80">
