...
// Reactive product list (kullanıcı silme/favori/günün şarkısı yaptığında liste güncellenebilir)
const productList = ref(props.products);

// Favori ürünlerin computed hali
const favoriteProducts = computed(() => productList.value.filter(p => p.is_favorite));

// Günün şarkısı yap
async function setDay(id) {
    try {
        await axios.post(`/songs/set-day/${id}`);
        // Günün Şarkısı sadece bir tane olur
...
async function toggleFavorite(id) {
    try {
        ...
        // Referans değişikliği için yeniden ata
...
<template>
    <div class="min-h-screen bg-pink-50 dark:bg-gray-900 py-10 px-4">

        <!-- Favori yoksa mesaj -->
        <div v-if="favoriteProducts.length === 0" class="text-center text-gray-500 mt-10">
          ...
          product.is_day ? 'border-4 border-pink-500' : 'border border-transparent'
            ...
            >
                ...
                <h4
                    ...
                    :class="product.is_day ? 'text-pink-600' : 'text-gray-900 dark:text-gray-100'"
                >
                    {{ product.name }}
                </h4>
              ...
              product.is_favorite
                ...
                <div
                    v-if="product.is_day"
                    class="absolute top-2 left-2 bg-pink-600 text-white px-2 py-1 rounded-full text-xs font-bold select-none shadow"
                >
                    🌞 Günün Şarkısı
                </div>
            </div>
